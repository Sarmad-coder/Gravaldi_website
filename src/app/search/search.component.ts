...
export class SearchComponent implements OnInit {
  ...
  allproducts() {
    this.productservice.getallProductslist(this.skipObj).subscribe(
      (data: any) => {
  ...
  setPageNumber(val) {
    this.loading_secondary = true;
    if (!this.miniLoading && this.skipObj.pageNumber != val && val > -1 && val < this.paegsnum) {
    ...
    if (val == 'All') {
      ...
      if (this.skipObj.style.length >= 1) {
        ...
        for (let i = 0; i < this.skipObj.style.length; i++) {
          if (this.skipObj.style[i] + '' === this.ProductStyles[val].obj.title + '') {
            found = true;
            index = i;
            break;
          }
    ...
    if (val == 'All') {
      ...
      if (this.skipObj.colors.length >= 1) {
        ...
        for (let i = 0; i < this.skipObj.colors.length; i++) {
          if (this.skipObj.colors[i] + '' === this.ProductColors[val].obj.title + '') {
            found = true;
            index = i;
            break;
          }
    ...
    if (val == 'All') {
      ...
      if (this.skipObj.patterns.length >= 1) {
        ...
        for (let i = 0; i < this.skipObj.patterns.length; i++) {
          if (this.skipObj.patterns[i] + '' === this.ProductPatterns[val].obj.title + '') {
            found = true;
            index = i;
            break;
          }
        ...
        if (found) {
          this.ProductPatterns[val].selected = !this.ProductPatterns[val].selected;
    ...
    if (val == 'All') {
      ...
      if (this.skipObj.fabric.length >= 1) {
        ...
        for (let i = 0; i < this.skipObj.fabric.length; i++) {
          if (this.skipObj.fabric[i] + '' === this.ProductFabrics[val].obj.title + '') {
            found = true;
            index = i;
            break;
          }
  ...
  remove_filter(val, index) {
    if (val == 'style') {
      let y = this.ProductStyles.findIndex(i => i.obj.title === this.skipObj.style[index])
      if (y != -1) {
        this.ProductStyles[index].selected = !this.ProductStyles[index].selected;
      }
      this.skipObj.style.splice(index, 1);
    } else if (val == 'fabric') {
      let y = this.ProductFabrics.findIndex(i => i.obj.title === this.skipObj.fabric[index])
      if (y != -1) {
        this.ProductFabrics[index].selected = !this.ProductFabrics[index].selected;
      }
      this.skipObj.fabric.splice(index, 1);
    } else if (val == 'patterns') {
      let y = this.ProductPatterns.findIndex(i => i.obj.title === this.skipObj.patterns[index])
      if (y != -1) {
        this.ProductPatterns[index].selected = !this.ProductPatterns[index].selected;
      }
      this.skipObj.patterns.splice(index, 1);
    } else if (val == 'colors') {
      let y = this.ProductColors.findIndex(i => i.obj.title === this.skipObj.colors[index])
      if (y != -1) {
        this.ProductColors[index].selected = !this.ProductColors[index].selected;
      }
      this.skipObj.colors.splice(index, 1);
    }
    this.serachOnFilter();
